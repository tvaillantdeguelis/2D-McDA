...
from legacy.calipso_constants import FILL_VALUE_FLOAT
from legacy.config import NB_PROF_EDGE, NB_PROF_OVERLAP, NB_PROF_SLICE
from legacy.detection.detect_features import detect_features
from legacy.detection.detect_surface import detect_surface
...
from legacy.merge.merge_masks import merged_feature_masks
...
if __name__ == '__main__':
    ...
    with timer("Apply algorithm by slice"):
        ...
        for i_slice in range(index_start_slice_array.size):
            with timer(
                f"Process slice {i_slice + 1:d}/{index_start_slice_array.size:d}"
            ):
                # Print slice number
                print(f"\n\n*****Slice {i_slice+1:d}/{index_start_slice_array.size:d}...*****") 
    ...
    # **********************************
    # *** Save data in netCDF-4 file ***
    print("\n\n############################################################\n"
          "*****Save data in netCDF file...*****")
    with timer("Save data in netCDF file"):
        request = ProcessingRequest(
            granule_date=GRANULE_DATE,
            caliop_version=VERSION_CAL_LID_L1,
            caliop_product_type=TYPE_CAL_LID_L1,
            current_directory=Path(FOLDER_PATH),
            previous_granule=PREVIOUS_GRANULE,
            previous_directory=(
                Path(PREVIOUS_FOLDER_PATH)
                if PREVIOUS_FOLDER_PATH is not None
                else None
            ),
            next_granule=NEXT_GRANULE,
            next_directory=(
                Path(NEXT_FOLDER_PATH)
                if NEXT_FOLDER_PATH is not None
                else None
            ),
            subset_mode=SLICE_START_END_TYPE,
            subset_start=SLICE_START,
            subset_end=SLICE_END,
            save_development_data=SAVE_DEVELOPMENT_DATA,
            output_version=VERSION_2D_McDA,
            output_product_type=TYPE_2D_McDA,
            output_directory=Path(OUT_FOLDER),
            maximum_altitude_index=INDEX30M_ALT_MAX,
        )
        result = ProcessingResult(
            data=data_dict_2d_mcda_all_slices,
            development=data_dict_2d_mcda_dev,
            altitude=cal_l1.get_data("Lidar_Data_Altitudes"),
            longitude_min=cal_l1.lon_min,
            longitude_max=cal_l1.lon_max,
        )
        write_product(request, result)
