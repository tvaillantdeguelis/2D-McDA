...
class CALIOPDerivedVariablesMixin:
    ...
    def _get_par_ab532(self, do_fillvalue):
        tot_ab_532 = self._get_native_values(
            "Total_Attenuated_Backscatter_532", do_fillvalue
        )
        per_ab_532 = self._get_native_values(
    ...
    def _get_nsf_in_ab_domain(self, wl, polar, do_fillvalue):
        sat_alt = self._get_native_values("Spacecraft_Altitude", do_fillvalue)[
            :, np.newaxis
        ]
        caliop_lidar_tilt = self._get_native_values("Off_Nadir_Angle", do_fillvalue)[
            :, np.newaxis
        ]
        data_alt = self._get_native_values("Lidar_Data_Altitudes", do_fillvalue)[
            np.newaxis, :
        ]
        range_alt = range_from_altitude(sat_alt, data_alt, caliop_lidar_tilt)
        pgr = np.array((1,))
        if wl == 532:
            calibration_cst = self._get_native_values(
                "Calibration_Constant_532", do_fillvalue
            )[:, np.newaxis]
            laser_energy = self._get_native_values("Laser_Energy_532", do_fillvalue)[
                :, np.newaxis
            ]
            if polar == "par":
                nsf = self._get_native_values(
            ...
            laser_energy = self._get_native_values("Laser_Energy_1064", do_fillvalue)[
                :, np.newaxis
            ]
            nsf = self._get_native_values("Noise_Scale_Factor_1064", do_fillvalue)[
                :, np.newaxis
            ]
        else:
            raise Exception(
    ...
    def _get_rms_in_ab_domain(self, wl, polar, do_fillvalue):
        ...
        if wl == 532:
            ...
            if polar == "par":
                rms = self._get_native_values(
                    "Parallel_RMS_Baseline_532", do_fillvalue
                )[:, np.newaxis]
                gain = self._get_native_values(
            ...
            rms = self._get_native_values("RMS_Baseline_1064", do_fillvalue)[
                :, np.newaxis
            ]
            gain = self._get_native_values("Amplifier_Gain_1064", do_fillvalue)[
                :, np.newaxis
            ]
        else:
            raise Exception(
