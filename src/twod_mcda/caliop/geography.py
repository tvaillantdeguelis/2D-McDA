...
def geo_distance(lat1, lon1, lat2, lon2):
    ...
    # Distance in rad
    dist_rad = acos(
        sin(np.deg2rad(lat1)) * sin(np.deg2rad(lat2))
        + cos(np.deg2rad(lat1))
        * cos(np.deg2rad(lat2))
        * cos(np.deg2rad(lon1) - np.deg2rad(lon2))
    )
...
def get_prof_min_max_indexes_from_latminmax(lat, lat_min, lat_max):
    ...
    if indices.size > 0:
        ...
        raise ValueError(
            f"No values found between lat_min={lat_min} and lat_max={lat_max}"
        )
...
if __name__ == "__main__":
    ...
    if False:
        ...
        with NetCDFReader(
            os.path.join(ERAI_PATH, "AN_SF", "2010", skt_filename)
        ) as data_reader:
        ...
        new_skin_temp = change_map_grid_resolution(
            skin_temp[0, :, :], lat, lon, new_lat, new_lon
        )
    ...
    if False:
        ...
        lon = np.array(
            (-170, -179, 175, 165, 155, 145, 20, 10, 5, -5, -10, -50, -100, -150, -160)
        )  # cross_combination 4 neg-pos-neg
