...
def change_detection_values(mask):
    ...
    new_mask[(new_mask == flag_likely_artifact) |\
             (new_mask == flag_AFA) |\
             (new_mask == flag_small_strips)] = flag_low_confidence
...
def merged_feature_masks(mask_532_par, mask_532_per, mask_1064):
    ...
    if not np.all((mask_532_par <= max_flag_detect) |\
                  (mask_532_par >= flag_small_strips)):
        sys.exit("mask_532_par has values not declared in global variables")
    if not np.all((mask_532_per <= max_flag_detect) |\
                  (mask_532_per >= flag_small_strips)):
        sys.exit("mask_532_per has values not declared in global variables")
    if not np.all((mask_1064 <= max_flag_detect) |\
                  (mask_1064 >= flag_small_strips)):
